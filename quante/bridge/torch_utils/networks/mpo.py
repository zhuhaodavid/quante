...
class MPO(TensorTrain):
    ...
    @classmethod
    def from_heisenberg(cls, L, j=1, h=0, cyclic=False, pauli=True, device='cpu') -> 'MPO':
        from ...generate.operas import heisenberg_operator
        ham = heisenberg_operator(L, j=j, h=h, cyclic=cyclic)
        return ham.to_mpo(pauli=pauli).to_torch(device)

    @classmethod
    def from_eye(cls, L, phys_dim=2, dtype=tc.float64, device='cpu') -> 'MPO':
        eyempo = [None] * L
    ...
    @classmethod
    def from_oper(cls, ham, L, pauli=True, device='cpu') -> 'MPO':
        return ham.to_mpo(L, pauli=pauli).to_torch(device)
