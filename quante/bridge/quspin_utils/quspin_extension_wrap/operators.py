...
# @Last Modified time: 2025-10-22 15:13:11
...
from ....generate.operas.spin import _convert2perm
...
def check_symm(oper, basis): 
    ...
    if isinstance(basis, spin_basis_1d):
        ...
        for k, (f,p1,p2,p3) in basis._user_maps.items():
            dic[k] = _convert2perm(L, f, p1, p2, p3)
        if basis._user_pcon_dict is not None:
            ...
            if nsf is not None:
                if nsf.__name__ == 'next_state_Np':
                    ...
                    dic['Ndiff'] = ((1<<basis._user_N)-1, [0])
                elif nsf.__name__ == 'next_state_Np_Nd':
                    dic['Nup2'] = ((1<<basis._user_N)-1, [[0]])
                else:
