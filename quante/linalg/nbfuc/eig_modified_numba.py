...
# @Last Modified time: 2025-06-11 22:59:37
...
@njit(["void(int32, int32, List(Set(int32)))", "void(int64, int64, List(Set(int64)))"])
...
def compute_blocks(ix, jx, d):  # pragma: no cover
    ...
    sectors : list[list[int]]
        ...
        block diagonalize the operator is then ``_np.concatenate(sectors)``.
...
def subselect(A, p):  # pragma: no cover
    ...
    """
    dp = len(p)
    out = _np.empty((dp, dp), dtype=A.dtype)
...
@njit
def _eigh_autoblocked(A, sort=True, dtpye=_np.float64):  # pragma: no cover
    d = A.shape[0]

    # allocate output arrays
    el = _np.empty(d, dtpye=dtpye)
    ev = _np.zeros_like(A)

    # find non-zero elements and group into charge sectors
    ix, jx = get_nz(A)
    gs = compute_blocks(ix, jx, d)
    gs = [_np.array(g) for g in gs]
    ...
    for i, g in enumerate(gs):
        ...
        mat = subselect(A, g)
        if mat.shape[0] > 32766:
            return None
        sub_el, sub_ev = _np.linalg.eigh(subselect(A, g))
...
@njit
def _eigvalsh_autoblocked(A, sort=True, dtpye=_np.float64):  # pragma: no cover
    # as above but ignore eigenvector for extra speed
    d = A.shape[0]

    el = _np.empty(d, dtype=dtpye)
