...
from .tensor_utils import clone
from .tensor_utils import eigh, qr, rq, svd, truncate, log_or_not_update, TruncationError


class TensorTrain:
    def __init__(self, Ws: list[np.ndarray], Ss: Optional[list[np.ndarray]] = None, llim: Optional[int] = None, rlim: Optional[int] = None, lognm: Optional[float] = None, L:Optional[int] = None) -> None:
        """
        .. code-block:: text
        ...
        """
        assert isinstance(Ws[0], np.ndarray)
        self.data = Ws
        
        self.L = len(Ws) if L is None else L
        assert self.L == len(Ws) or self.L == np.inf
        ...
        self.lognm = lognm if lognm is not None else np.array(0.0, dtype=np.float64)
    ...
    def add_(self, anotherTT):
        ...
        newlognm = np.logaddexp(a, b)
        # 这样 alpha, beta 都在 0-1 之间
        alpha = np.exp(a - newlognm)
        beta = np.exp(b - newlognm)
        # 跟新数据
    ...
    def __rmul__(self, number:np.ndarray):
        """乘以数，为了效率，不复制张量！！！
        ...
        Examples
        --------
        >>> 𝜓1 = tn.MPS.random(4, linkdims=3, dtype=np.float64)
    ...
    def to(self, dtype=None):
        for i in range(len(self.data)):
            if dtype is not None:
                self.data[i] = self.data[i].astype(dtype, copy=False)
        for i in range(len(self.data)+1):
            if dtype is not None and self.Ss[i] is not None:
                self.Ss[i] = self.Ss[i].astype(dtype, copy=False)
        if dtype is not None:
    ...
    def inner(self, anotherTT, logscale=False, conj=True):
        ...
        if logscale:
            ...
            return np.log(coef) + lognm + self.lognm + anotherTT.lognm
        else:
            return tf.tn_inner(conjdata, anotherTT.data, logscale=False) * np.exp(self.lognm) * np.exp(anotherTT.lognm)

    def norm(self, lognorm=False):
        if self.is_canonical_form():
            return self.lognm if lognorm else np.exp(self.lognm)
        elif self.is_orthogonal_form():
            nm = np.linalg.norm(self.data[self.llim])
            if lognorm:
                return np.log(nm) + self.lognm
            else:
                return nm * np.exp(self.lognm)
        else:
            if lognorm:
                coef, lognm = tf.tn_norm(self.data, lognorm=True)
                return np.log(coef) + lognm + self.lognm
            else:
                coef = tf.tn_norm(self.data, lognorm=False)
                return coef * np.exp(self.lognm)
    ...
    def entanglement_entropy(self, bonds: list[int]):
        if self.is_canonical_form():
            ...
            for bond in range(bonds):
                s = self.Ss[bond]
                assert np.linalg.norm(s) - 1 < 1E-8
                res.append(-np.sum(s**2 * np.log(s**2)))
            return res
        ...
        for bond in bonds:
            ...
            data = psi.data[bond]
            _, s, _ = scipy.linalg.svd(data.reshape(data.shape[0], -1))
            assert np.linalg.norm(s) - 1 < 1E-8
            res.append(-np.sum(s**2 * np.log(s**2)))
    ...
    def check_canonical_form(self):
        for i in range(len(self.data)):
            ...
            assert np.allclose(tmp @ tmp.conj().T, np.eye(dim, dtype=tmp.dtype)), f"第 {i} 个张量不是正交的, diff = {np.linalg.norm(tmp @ tmp.conj().T, np.eye(dim, dtype=tmp.dtype))}"
    ...
    def check_orthogonal_form(self):
        ...
        for i in range(self.llim):
            ...
            assert np.allclose(tmp.conj().T @ tmp, np.eye(dim, dtype=tmp.dtype)), f"第 {i} 个张量不是正交的"
        
        for i in range(self.rlim+1, len(self.data)):
            dim = self.data[i].shape[0]
            tmp = self.data[i].reshape(dim, -1)
            assert np.allclose(tmp @ tmp.conj().T, np.eye(dim, dtype=tmp.dtype)), f"第 {i} 个张量不是正交的"
    ...
    def orthogonalize_(self, j: int, normalize=True):
        ...
        self.move_rlim_(j)
        if normalize:
            oldnm = np.linalg.norm(self.data[j])
            self.data[j] = self.data[j] / oldnm
            self.lognm = self.lognm + np.log(oldnm)
    ...
    def apply_gate_(
        self,
        pos:int,
        gate:Union[np.ndarray, tuple[np.ndarray, str]],
        *,
        direction: str = None,
        svd_alg: str = "svd", 
        eigdirection: str = None,
        pertube: np.ndarray = None,
    ...
    def _convert_gate(self, gate, site_num):
        ...
        #             self.to(np.complex128)
        #     else:
        #         if np.issubdtype(self.dtype, np.complexfloating):
        #             gate = np.array(gate, dtype=np.complex128)
        #         else:
        #             gate = np.array(gate, dtype=np.float64)
        # else:
        #     if np.issubdtype(gate.dtype, np.complexfloating) and not np.issubdtype(self.dtype, np.complexfloating):
        #         self.to(np.complex128)
        #     elif not np.issubdtype(gate.dtype, np.complexfloating) and np.issubdtype(self.dtype, np.complexfloating):
        #         gate = gate.astype(np.complex128, copy=False)
        return gate
    ...
    def update_two_site_(self, pos, W, direction, *,
        ...
        # -------------- 使用 eig ------------
            ...
            if updateS:
                self.Ss[pos + 1] = (S if not normalize else S / np.linalg.norm(S))
    ...
    def update_two_site_cannonical_(self, pos, W,
        ...
        if svd_alg == 'eig':
            ...
            self.data[pos] = W1
            self.Ss[next_pos], self.data[next_pos] = S/np.linalg.norm(S) if normalize else S, W2
    ...
    def _partition(self, startpos, endpos):
        ...
        for i in range(startpos, endpos+1):
            newtt.append(self.data[i].copy())
            newSs.append(self.Ss[i].copy() if self.Ss[i] is not None else None)
    ...
    def swapsite_(self, i, j, direction='right', move_center=False, **kwargs):
        ...
        if contracted_tsr.ndim == 4:
            contracted_tsr = contracted_tsr.transpose(0, 2, 1, 3)
        else:
            contracted_tsr = contracted_tsr.transpose(0, 3, 4, 1, 2, 5)
        
        self.update_two_site_(i, contracted_tsr, direction=direction, **kwargs)


def _local_operator(gate, dims, pos, site_num):
    """构造作用在完整 Hilbert 空间上的局部门矩阵。"""
    before = int(np.prod(dims[:pos]))
