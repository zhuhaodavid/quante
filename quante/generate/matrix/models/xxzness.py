...
# @Last Modified time: 2026-05-23 09:58:04
...
The amplitude operator Ω is upper/lower trianglar
...
Auxiliary matrices A_sigma in Eq. (2.31):
    ...
    A_+ = sum_k b_k |k><k+1|
        ...
        A_+ moves one step down in the ket label:
    ...
    A_- = sum_k |k+1><k|
        ...
        A_- moves one step up in the ket label:

              |k> --A_---> |k+1>

The coefficients a_k and b_k are generated by the recurrence relations
Eqs. (2.32)-(2.34):

    a_{k+1} = 2 Delta a_k - a_{k-1}
...
and 
    ...
    b_{k+1} = b_k + 2 a_{k+1} (Delta a_{k+1} - a_k)

with
    b_0 = i eps

In this implementation Delta is ``delta`` and eps is ``eps``.
"""
