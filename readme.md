...
qt.basicfun.isave("data.h5", a, group='/')
```

将建立一个 `data.h5` 文件，包含一个 `/` 组，组中包含一个 `a` 数据集。

通过 `iload` 函数可以加载数据。

```python
a = qt.basicfun.iload("data.h5", group='/')
```
...
```
MPS;  torch.float64;  norm: 4.669e+04;  maxbonddim: 4;  device: cpu;
