...
- `conda install numba scipy matplotlib h5py cytoolz psutil`, basic package dependence.

- `conda install mypy matplotlib tqdm ipykernel ipympl`, packages often used.

- `conda install --channel=conda-forge physics-tenpy` tenpy

- `nvcc --version`, check version `release xxx`. https://pytorch.org/ `conda install pytorch ...`, according to CUDA version. For cpu,  `conda install pytorch torchvision torchaudio cpuonly -c pytorch`

- Add the `parent path` of `quante` to a `path.pth` file. And bring that file into `anaconda3/envs/envname/lib/python3.11/site-packages` folder.

- `import sys; print(sys.path)`, check whether the quante path in.


## package dependence (python=3.10) for quspin

newest version of quspin support python 3.10,

the following installation will make both quante and quspin usable as the same time.

- `conda create -n envname python=3.10`, create new virtual python envrionment.

- `conda activate envname`

- `conda install -c weinbe58 omp quspin=0.3.7 numpy=1.21 numba=0.56`

- `conda install h5py cytoolz psutil`

the follwing packages are often used but not required for running quspin and quante

- `conda install matplotlib tqdm mypy ipykernel ipympl`
